  ...
  */
  function leaderboardSwitcher() {
    try {
      ...
      $(document).on('click', '.leaderboard-details .mecha-card-v2 .btn-close', function () {
      ...
      $(document).on('click', '.leaderboard-list .leaderboard-card', function () {
        let card = $(this),
          ...
          stats = card.find('.leaderboard-card__stats-wrapper').children().map((_, stat) => ({
